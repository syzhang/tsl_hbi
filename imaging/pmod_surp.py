"""IO jump freq model pmod surprise"""
...
def subjectinfo(subject_id):
    ...
    for r in range(len(runs)):
        ...
        # demean pmod
        param_list = list(df_sj['pmod_surprise'].values - np.mean(df_sj['pmod_surprise'].values)) 
        pmod_bunch = [Bunch(name=['ps'], poly=[1], param=[param_list]),None]
...
if __name__ == "__main__":
    ...
    output_1st_dir = '1stLevel_' + model_name + '_surp'
    print(f'current subject: {subject_list}, model: {model_name}')

    # define contrasts
    # pmod contrasts
    cont01 = ['psurp_p1', 'T', ['Stimxps^1'], [1]]
    cont02 = ['psurp_np1', 'T', ['Stimxps^1'], [-1]]
