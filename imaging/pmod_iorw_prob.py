"""IO jump freq model pmod (prob and surprise)"""
...
def subjectinfo(subject_id):
    ...
    for r in range(len(runs)):
        ...
        # demean pmod - io prob
        param_list_io = list(df_sj_io['pmod'].values - np.mean(df_sj_io['pmod'].values)) 
        # demean pmod - rw prob
        param_list_rw = list(df_sj_rw['pmod'].values - np.mean(df_sj_rw['pmod'].values)) 
        pmod_bunch = [Bunch(name=['io_prob'], poly=[1], param=[param_list_io]), Bunch(name=['rw_prob'], poly=[1], param=[param_list_rw]),None]
#         pmod_bunch = None
...
if __name__ == "__main__":
    ...
    # define contrasts
    con_names = ['Stim_ioxio_prob^1','Stim_rwxrw_prob^1' ]
    # pmod contrasts
    cont01 = ['pprob_io', 'T', con_names, [1,0]]
    cont02 = ['pprob_nio', 'T', con_names, [-1,0]]
    cont03 = ['pprob_rw', 'T', con_names, [0,1]]
    cont04 = ['pprob_nrw', 'T', con_names, [0,-1]]
