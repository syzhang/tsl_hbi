"""IO jump freq model pmod with both posterior and kl divergence as pe, with high/low pain confounds with ones as pmod"""
...
def subjectinfo(subject_id):
    ...
    for r in range(len(runs)):
        ...
        pmod_bunch = [Bunch(name=['pmod_mean'], poly=[1], param=[param_list_pp]),
                    Bunch(name=['pmod_pe'], poly=[1], param=[param_list_pe]),
                    # Bunch(name=['high_ones'], poly=[1], param=[list(np.ones(len(onset_list[r*conds_num+2])))]),
                    # Bunch(name=['low_ones'], poly=[1], param=[list(np.ones(len(onset_list[r*conds_num+3])))]),
                    None]
#         pmod_bunch = None
...
if __name__ == "__main__":
    ...
    # define contrasts
    con_names = ['Stim_ppxpmod_mean^1','Stim_pexpmod_pe^1', 'Highxhigh_ones^1','Lowxlow_ones^1']
    # pmod contrasts
    ...
    cont08 = ['High>Low','T', con_names, [0,0,1,-1]]
    cont09 = ['Low>High','T', con_names, [0,0,-1,1]]
